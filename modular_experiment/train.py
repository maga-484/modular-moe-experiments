...
def train(max_num=4, modulo=None, num_experts=4, epochs=200, 
    ...
    # Datasets
    train_dataset = ModularAdditionDataset(max_num=max_num, modulo=modulo, 
                                           size=10000, seed=seed)
    val_dataset = ModularAdditionDataset(max_num=max_num, modulo=modulo,
    ...
    save_name = f"model_modulo_{modulo if modulo is not None else 'none'}_experts_{num_experts}.pt"
    
    print(f"Entrenando con modulo={modulo}, num_experts={num_experts}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--modulo', type=parse_modulo, default=None, 
                       help='Módulo para la suma (usa "none" para suma normal)')
