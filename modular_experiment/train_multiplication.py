...
from modular_experiment.dataset_mul import ModularMultiplicationDataset
...
def train(max_num=4, modulo=None, num_experts=4, epochs=200, 
    ...
    # Datasets
    train_dataset = ModularMultiplicationDataset(max_num=max_num, modulo=modulo, 
                                                   size=10000, seed=seed)
    val_dataset = ModularMultiplicationDataset(max_num=max_num, modulo=modulo,
    ...
    op_name = "mul" if modulo is None else f"mul_mod{modulo}"
    save_name = f"model_{op_name}_experts_{num_experts}.pt"
    
    print(f"Entrenando multiplicación con modulo={modulo}, num_experts={num_experts}")
...
def parse_modulo(value):
    """Convertir argumento de módulo: 'none' o 'None' -> None, else int"""
    if value.lower() == 'none':
        return None
    return int(value)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--modulo', type=parse_modulo, default=None, 
                       help='Módulo para multiplicación (usa "none" para multiplicación normal)')
    parser.add_argument('--num_experts', type=int, default=4, help='Número de expertos')
    parser.add_argument('--epochs', type=int, default=200, help='Número de épocas')
    parser.add_argument('--hidden_dim', type=int, default=64, help='Dimensión de capas ocultas')
